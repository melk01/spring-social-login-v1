...
public class ApplicationTest {
    ...
    public void homePageRedirects() throws Exception {
        mockMvc.perform(get("/myURL")).andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/login"));
    }

    @Test
    public void connectPageHasLink() throws Exception {
        ResultActions rAction = mockMvc.perform(get("/connect"));
        rAction.andExpect(status().isOk());
        rAction.andExpect(content().string(containsString("Connect to Facebook")));
    }
